...
----
	...
	class PersonController {
		...
		public void add(@RequestBody Person man) {
			// ...
		}
	}
----
...
----
	...
	class PersonController {
		...
		@PostMapping
		@ResponseStatus(HttpStatus.CREATED)
		fun add(@RequestBody man: Person) {
			// ...
		}
	}
----
...
controller mapped to `/man` is also implicitly mapped to `/man.{asterisk}`.
The file extension is then used to interpret the requested content type to use for
the response (that is, instead of the `Accept` header) -- for example, `/man.pdf`,
`/man.xml`, and others.
