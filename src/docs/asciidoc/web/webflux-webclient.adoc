...
.Java
----
	Person man = client.get().uri("/man/{id}", i).retrieve()
		.bodyToMono(Person.class)
		.block();
...
----
	val man = runBlocking {
		client.get().uri("/man/{id}", i).retrieve()
				.awaitBody<Person>()
	}
...
.Java
----
	Mono<Person> personMono = client.get().uri("/man/{id}", personId)
			.retrieve().bodyToMono(Person.class);

	Mono<List<Hobby>> hobbiesMono = client.get().uri("/man/{id}/hobbies", personId)
			.retrieve().bodyToFlux(Hobby.class).collectList();

	Map<String, Object> data = Mono.zip(personMono, hobbiesMono, (man, hobbies) -> {
...
----
	val data = runBlocking {
			val personDeferred = async {
				client.get().uri("/man/{id}", personId)
						.retrieve().awaitBody<Person>()
			}

			val hobbiesDeferred = async {
				client.get().uri("/man/{id}/hobbies", personId)
						.retrieve().bodyToFlow<Hobby>().toList()
			}

			mapOf("man" to personDeferred.await(), "hobbies" to hobbiesDeferred.await())
		}
----
